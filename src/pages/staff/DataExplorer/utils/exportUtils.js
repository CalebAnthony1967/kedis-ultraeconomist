...
export async function exportChartAsPNG(elementRef, filename = 'chart.png') {
  ...
  try {
    // Dynamic import to avoid build issues if html2canvas is not installed
...
export function getShareableURL(filters) {
  ...
  if (filters.entityLevels?.length) params.set('el', filters.entityLevels.join(','));
...
export function parseShareableURL() {
  ...
  if (params.has('el')) filters.entityLevels = params.get('el').split(',').filter(Boolean);
